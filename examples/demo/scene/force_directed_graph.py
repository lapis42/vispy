...
typ = 0
while ptr < npts:
    size = int(np.random.random() * grpsize + grpsize // 2)
...
markers = scene.Markers(pos=pos, face_color=colors, symbol='square', size=20,
                        parent=view.scene)
index = scene.Text(texts, pos = pos, italic=False, bold=False,
                       color='w', font_size=8, parent=view.scene) 
...
def update(ev):
    ...
    markers.set_data(pos=pos, symbol='square', face_color=colors)
